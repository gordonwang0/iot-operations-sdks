...
1. **Where to expose the executor** — on `AssetClient` or a new dedicated `ManagementActionClient`?
   - **Decision: New dedicated `ManagementActionClient`.**
   - Rationale: Management action execution is inbound (receive RPC requests, dispatch, respond), fundamentally different from `AssetClient`'s outbound concerns (forward data, report health/status). Each management action has its own lifecycle (create/update/delete, executor replacement) that doesn't map to the "one client per asset" model. A dedicated type keeps both types focused and manageable. Health reporting remains on `AssetClient` (already implemented, consistent with datasets/events/streams pattern).
   - Delivery: A new callback or notification mechanism on `ConnectorWorker` will surface `ManagementActionClient` instances to user code. Precedent: `DeviceEndpointClient` is already delivered alongside `AssetClient` in `AssetAvailableEventArgs`.
2. **Notification model** — how to surface lifecycle events (C# events, async enumerable, callback pattern)?
   - **Decision: Hybrid approach — `WhileManagementActionIsAvailable` callback + fine-grained notifications inside.**
   - Outer lifecycle: A `Func<ManagementActionAvailableEventArgs, CancellationToken, Task>` callback on `ConnectorWorker`, matching the existing `WhileAssetIsAvailable` / `WhileDeviceIsAvailable` pattern. The `CancellationToken` is cancelled only on true termination (delete, leadership loss, shutdown) — not on every update.
   - Inner notifications: The `ManagementActionClient` inside the event args exposes fine-grained lifecycle notifications for updates (Updated, UpdatedWithNewExecutor, AssetUpdated) without tearing down the whole callback. This preserves the Rust `recv_notification()` semantics where non-destructive updates don't destroy user state.
   - Request reception: The executor provides `RecvRequestAsync()` for receiving management action invocations. Users coordinate requests and notifications inside the callback (e.g., via `Task.WhenAny` or similar select-style pattern).
   - Rationale: Keeps the connector layer consistent (callback pattern users already know), while accommodating management action's unique per-action lifecycle (executor replacement, non-destructive updates). Matches how the Rust sample works: outer spawned task per action, inner `select!` loop for notifications + requests.
...
4. **Schema reporting integration** — extend `AssetClient` or new surface?
   - **Decision: On `ManagementActionClient` (Option A, Rust-aligned).**
   - Rationale: Management actions have a request/response schema pair that is per-action, not per-asset. There is no data-forwarding trigger to piggyback on (unlike datasets/events where `ForwardSampledDatasetAsync` implicitly registers schemas via `ConnectorWorker`). The client that owns the action's lifecycle should own its schemas. Matches Rust 1:1 for cross-SDK consistency.
   - Note: This means management action schema reporting is explicit (user calls `ReportRequestMessageSchemaAsync` / `ReportResponseMessageSchemaAsync`), while dataset/event schemas are implicit (registered as a side effect of forwarding). This inconsistency is acceptable — it reflects the real difference in data flow (inbound RPC vs outbound telemetry). The explicit approach is arguably more transparent.
   - Methods on `ManagementActionClient`: `ReportRequestMessageSchemaAsync`, `ReportResponseMessageSchemaAsync`, `ReportRequestMessageSchemaReferenceAsync`, `ReportResponseMessageSchemaReferenceAsync`. The client will need a `SchemaRegistryClient` dependency (simple MQTT-backed client already used by `ConnectorWorker`).
